...
int getNextElement(int series[], int numElements) {
    ...
    if (allZeroes == 1) {
        return series[numElements-1];
    } else {
        return series[numElements-1] + getNextElement(newSeries, numElements-1);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        int nextElement = getNextElement(series, SERIES_LEN);
